...
def get_most_frequent(ciphertext):
    ...
    return letter_count  # Return full counts for all letterscounts
...
def calc_ioc(col_text):
    ...
    frequencies = get_most_frequent(col_text)
...
def preprocess(ciphertext):
    ...
    '''
    #return ''.join([char.lower() for char in ciphertext if char.isalpha()])
    cleaned = ''.join([char.lower() for char in ciphertext if char.isalpha()])
   # print(f"Cleaned Ciphertext: {cleaned}")  #                  ---------------------------------------- debugging
    return cleaned
    '''
...
if __name__ == "__main__":
    ...
    if isinstance(result, tuple) and result[0]:  # if key found
        (f"Key: {result[0]} | Decrypted Text: {result[1]}")
    else:
